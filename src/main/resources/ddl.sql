...
CREATE TABLE IF NOT EXISTS `navigators_db`.`segments` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `point_a_id` BIGINT UNSIGNED NOT NULL,
  `point_b_id` BIGINT UNSIGNED NOT NULL,
  `length` INT NOT NULL,
  INDEX `fk_distances_points1_idx` (`point_a_id` ASC) VISIBLE,
  INDEX `fk_distances_points2_idx` (`point_b_id` ASC) VISIBLE,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_distances_points1`
    FOREIGN KEY (`point_a_id`)
    REFERENCES `navigators_db`.`points` (`id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_distances_points2`
    FOREIGN KEY (`point_b_id`)
    REFERENCES `navigators_db`.`points` (`id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
