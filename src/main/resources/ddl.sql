...
CREATE TABLE IF NOT EXISTS `navigators_db`.`segment_points` (
  ...
  `point_id` BIGINT UNSIGNED NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_segments_has_points_points1_idx` (`point_id` ASC) VISIBLE,
  INDEX `fk_segments_has_points_segments1_idx` (`segment_id` ASC) VISIBLE,
  CONSTRAINT `fk_segments_has_points_segments1`
    FOREIGN KEY (`segment_id`)
    REFERENCES `navigators_db`.`segments` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_segments_has_points_points1`
    FOREIGN KEY (`point_id`)
    REFERENCES `navigators_db`.`points` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `navigators_db`.`route_segments` (
  `id` INT NOT NULL,
  `segments_id` BIGINT UNSIGNED NOT NULL,
  `routes_id` BIGINT UNSIGNED NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_segments_has_routes_routes1_idx` (`routes_id` ASC) VISIBLE,
  INDEX `fk_segments_has_routes_segments1_idx` (`segments_id` ASC) VISIBLE,
  CONSTRAINT `fk_segments_has_routes_segments1`
    FOREIGN KEY (`segments_id`)
    REFERENCES `navigators_db`.`segments` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_segments_has_routes_routes1`
