...
CREATE TABLE IF NOT EXISTS `navigators_db`.`trips` (
  ...
  `finish_point_id` BIGINT UNSIGNED NOT NULL,
  `routes_id` BIGINT UNSIGNED NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_trips_navigators_idx` (`navigator_id` ASC) VISIBLE,
  INDEX `fk_trips_points1_idx` (`start_point_id` ASC) VISIBLE,
  INDEX `fk_trips_points2_idx` (`finish_point_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `navigators_db`.`bus_route_points` (
  ...
  `point_id` BIGINT UNSIGNED NOT NULL,
  INDEX `fk_bus_routes_has_points_points1_idx` (`point_id` ASC) VISIBLE,
  INDEX `fk_bus_routes_has_points_bus_routes1_idx` (`bus_route_id` ASC) VISIBLE,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_bus_routes_has_points_bus_routes1`
    FOREIGN KEY (`bus_route_id`)
    REFERENCES `navigators_db`.`bus_routes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_bus_routes_has_points_points1`
    FOREIGN KEY (`point_id`)
    REFERENCES `navigators_db`.`points` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
